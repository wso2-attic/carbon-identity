...
CREATE TABLE SP_IDP
  (
    SP_IDP_ID            INTEGER NOT NULL,
    SP_TENANT_ID         INTEGER NOT NULL,
...
CREATE TRIGGER IDP_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL 

BEGIN ATOMIC
    
    SET (NEW.SP_IDP_ID)
       = (NEXTVAL FOR IDP_SEQUENCE);

END/

CREATE TABLE SP_IDP_ROLES
  (
    SP_IDP_ROLE_ID INTEGER NOT NULL,
    SP_IDP_ID      INTEGER NOT NULL,
    SP_IDP_ROLE    VARCHAR(254) NOT NULL,
...
CREATE TRIGGER IDP_ROLES_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_ROLES
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_ROLE_ID)
       = (NEXTVAL FOR IDP_ROLES_SEQUENCE);

END/

CREATE TABLE SP_IDP_ROLE_MAPPINGS
  (
    SP_IDP_ROLE_MAPPING_ID INTEGER NOT NULL,
    SP_IDP_ROLE_ID         INTEGER NOT NULL,
    SP_TENANT_ID           INTEGER NOT NULL,
    SP_USER_STORE_ID       VARCHAR (253) NOT NULL,
    SP_LOCAL_ROLE          VARCHAR(253) NOT NULL,
...
CREATE TRIGGER IDP_ROLE_MAPPINGS_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_ROLE_MAPPINGS
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_ROLE_MAPPING_ID)
       = (NEXTVAL FOR IDP_ROLE_MAPPINGS_SEQUENCE);

END/

CREATE TABLE SP_IDP_CLAIMS
  (
    SP_IDP_CLAIM_ID               INTEGER NOT NULL, 
    SP_IDP_ID                     INTEGER NOT NULL,
    SP_IDP_CLAIM                  VARCHAR(254) NOT NULL,
...
CREATE TRIGGER IDP_CLAIMS_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_CLAIMS
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_CLAIM_ID)
       = (NEXTVAL FOR IDP_CLAIMS_SEQUENCE);

END/

CREATE TABLE SP_IDP_CLAIM_MAPPINGS
  (
    SP_IDP_CLAIM_MAPPING_ID INTEGER NOT NULL,
    SP_IDP_CLAIM_ID         INTEGER NOT NULL,
    SP_TENANT_ID            INTEGER NOT NULL,
    SP_LOCAL_CLAIM          VARCHAR(253) NOT NULL,
...
CREATE TRIGGER IDP_CLAIM_MAPPINGS_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_CLAIM_MAPPINGS
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_CLAIM_MAPPING_ID)
       = (NEXTVAL FOR IDP_CLAIM_MAPPINGS_SEQUENCE);

END/

CREATE TABLE SP_IDP_OPENID
  (
    SP_IDP_OPENID_ID            INTEGER NOT NULL,
    SP_IDP_ID                   INTEGER,
...
CREATE TRIGGER IDP_OPENID_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_OPENID
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_OPENID_ID)
       = (NEXTVAL FOR IDP_OPENID_SEQUENCE);

END/

CREATE TABLE SP_IDP_SAMLSSO
  (
    SP_IDP_SAMLSSO_ID            INTEGER NOT NULL,
    SP_IDP_ID                    INTEGER,
...
CREATE TRIGGER IDP_SAMLSSO_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_SAMLSSO
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_SAMLSSO_ID)
       = (NEXTVAL FOR IDP_SAMLSSO_SEQUENCE);

END/

CREATE TABLE SP_IDP_OIDC
  (
    SP_IDP_OIDC_ID            INTEGER NOT NULL,
    SP_IDP_ID                 INTEGER,
...
CREATE TRIGGER IDP_OIDC_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_OIDC
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_OIDC_ID)
       = (NEXTVAL FOR IDP_OIDC_SEQUENCE);

END/

CREATE TABLE SP_IDP_FB_AUTH
  (
    SP_IDP_FB_AUTH_ID       INTEGER NOT NULL,
    SP_IDP_ID               INTEGER,
...
CREATE TRIGGER IDP_FB_AUTH_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_FB_AUTH
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_FB_AUTH_ID)
       = (NEXTVAL FOR IDP_FB_AUTH_SEQUENCE);

END/

CREATE TABLE SP_IDP_PASSIVE_STS
  (
    SP_IDP_PASSIVE_STS_ID         INTEGER NOT NULL,
    SP_IDP_ID                     INTEGER,
...
CREATE TRIGGER IDP_PASSIVE_STS_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_PASSIVE_STS
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_PASSIVE_STS_ID)
       = (NEXTVAL FOR IDP_PASSIVE_STS_SEQUENCE);

END/

CREATE TABLE SP_IDP_PROVISIONING_CONFIGS 
  (
                        SP_IDP_PROV_CONFIG_ID INTEGER NOT NULL,
                        SP_IDP_ID INTEGER,
...
/
CREATE SP_IDP_PROVISIONING_CONFIGS_SEQUENCE
START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE TRIGGER SP_IDP_PROVISIONING_CONFIGS_TRIGGER NO CASCADE BEFORE INSERT ON SP_IDP_PROV_CONFIG_ID 
REFERENCING NEW AS NEW FOR EACH ROW MODE DB2SQL

BEGIN ATOMIC

    SET (NEW.SP_IDP_PROV_CONFIG_ID)
       = (NEXTVAL FOR SP_IDP_PROVISIONING_CONFIGS_SEQUENCE);

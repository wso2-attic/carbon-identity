...
import org.wso2.carbon.identity.application.mgt.*;
...
public class ApplicationManagementServiceComponent {
	...
	private static Map<String, ServiceProvider> fileBasedSPs =
	                                                           new HashMap<String, ServiceProvider>();
	...
	protected void activate(ComponentContext context) {
		// Registering ApplicationManagementService as a OSGIService
	...
	/**
	 * Build file based service providers
	 */
	private void buildFileBasedSPList() {
		String spConfigDirPath =
			...
			for (final File fileEntry : spConfigDir.listFiles()) {
				try {
					if (!fileEntry.isDirectory()) {
						fileInputStream =
						                  new FileInputStream(new File(fileEntry.getAbsolutePath()));
	...
	/**
	 * Get file based service providers
	 *
	 * @return file based service provider map
	 */

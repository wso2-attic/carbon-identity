                                    ...
                                    <td><select onchange="loadClaims()" id="claimDialects"
                                        ...
                                        <%
                                            if (claimMapping != null) {
                                                for (ClaimDialectDTO claimDialectDTO : claimAdminClient
                                                        .getAllClaimMappings()) {
                                                    if (claimDialectDTO.getDialectURI().trim().equals(dialect.trim())) {
                                        %>
                                        <option name="dialect" selected="true"
                                                value="<%=claimDialectDTO.getDialectURI()%>"><%=claimDialectDTO
                                                .getDialectURI()%>
                                        </option>
                                        <%
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <table style="margin-left:-10px;">
                                            <%
                                                if (claimMapping != null && claimMapping.getClaimMappings() != null &&
                                                    claimMapping.getClaimMappings().length > 0) {
                                                    for (ClaimMappingDTO dto : claimMapping.getClaimMappings()) {
                                            %>
                                            <tr>
                                                <td>
                                                    <input type="checkbox" name="claims"
                                                           value="<%=dto.getClaim().getClaimUri()%>"/><%=dto.getClaim()
                                                                                                            .getClaimUri()%>
                                                </td>
                                            </tr>
                                            <%

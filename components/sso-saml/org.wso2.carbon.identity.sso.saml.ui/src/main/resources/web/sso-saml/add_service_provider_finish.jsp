...
<script>
    ...
    location.href = '../application/configure-service-provider.jsp?action=update&display=samlIssuer&spName=' +
            '<%=Encode.forUriComponent(spName)%>&samlIssuer=<%=Encode.forUriComponent(serviceProviderDTO.getIssuer())%>' +
            '&attrConServIndex=<%=Encode.forUriComponent(attributeConsumingServiceIndex)%>';
    <% } else { %>
    location.href = '../application/configure-service-provider.jsp?action=delete&display=samlIssuer&spName=' +
            '<%=spName%>&samlIssuer=<%=serviceProviderDTO.getIssuer()%>&attrConServIndex=' +
            '<%=Encode.forUriComponent(attributeConsumingServiceIndex)%>';

...
/**
 * This class access the IDN_APPMGT database to store/update and delete
 * application configurations. The IDN_APPMGT database contains few tables
 * <ul>
...
public class ApplicationDAOImpl implements ApplicationDAO {
	...
	private void addServiceProviderProperties(Connection dbConnection, int spId,
			List<ServiceProviderProperty> properties, int tenantId) throws SQLException {
	...
	/**
	 * Stores basic application information and meta-data such as the
	 * application name, creator and tenant.
	 *
	...
	public int createApplication(ServiceProvider serviceProvider, String tenantDomain)
		...
		try {
			...
			// default authenticator is defined system-wide - in the
			// configuration file.
	...
	public void updateApplication(ServiceProvider serviceProvider, String tenantDomain)
		...
		try {
			if (ApplicationManagementServiceComponent.getFileBasedSPs()
					.containsKey(serviceProvider.getApplicationName())) {
			...
			updateOutboundProvisioningConfiguration(applicationId, serviceProvider.getOutboundProvisioningConfig(),
					connection);
			...
			try {
				...
				throw new IdentityApplicationManagementException("Failed to update service provider " + applicationId,
						e);
			}
	...
	private void updateBasicApplicationData(ServiceProvider serviceProvider, Connection connection)
		...
		if (!StringUtils.equals(applicationName, storedAppName)) {
			String applicationNameforRole = IdentityUtil.addDomainToName(applicationName,
					ApplicationConstants.APPLICATION_DOMAIN);
			String storedAppNameforRole = IdentityUtil.addDomainToName(storedAppName,
					ApplicationConstants.APPLICATION_DOMAIN);
			// rename the role
			...
			for (Map.Entry<String, String> entry : applicationPermissions.entrySet()) {
				updatePermissionPath(entry.getKey(),
						entry.getValue().replace(storedAppName.toLowerCase(), applicationName.toLowerCase()));
			}
		}
		...
		PreparedStatement storeAppPrepStmt = null;
		try {
			storeAppPrepStmt = connection.prepareStatement(ApplicationMgtDBQueries.UPDATE_BASIC_APPINFO);
			// SET APP_NAME=?, DESCRIPTION=? IS_SAAS_APP=? WHERE TENANT_ID= ?
			// AND ID = ?
		...
		try {
			...
			for (InboundAuthenticationRequestConfig authRequest : authRequests) {
				if (authRequest == null || authRequest.getInboundAuthKey() == null
					...
					// not a valid authentication request. Must have client and
					// a type.
					continue;
				}
				// TENANT_ID, INBOUND_AUTH_KEY,INBOUND_AUTH_TYPE,PROP_NAME,
				// PROP_VALUE, APP_ID
	...
	private void updateInboundProvisioningConfiguration(int applicationId,
			InboundProvisioningConfig inBoundProvisioningConfig, Connection connection) throws SQLException {
	 ...
	 * @throws SQLException
	 */
	private InboundProvisioningConfig getInboundProvisioningConfiguration(int applicationId, Connection connection,
	 ...
	 * @throws SQLException
	 */
	private OutboundProvisioningConfig getOutboundProvisioningConfiguration(int applicationId, Connection connection,
			int tenantID) throws SQLException {
		...
		try {
			...
			outBoundProvisioningConfig.setProvisioningIdentityProviders(
					idpProConnectors.toArray(new IdentityProvider[idpProConnectors.size()]));
	...
	private void updateLocalAndOutboundAuthenticationConfiguration(int applicationId,
		...
		try {
			...
			// IS_USE_USERSTORE_DIMAIN_LOCAL_SUBJECT_ID=? WHERE TENANT_ID= ? AND
			// ID = ?
		...
		if (authSteps == null || authSteps.length == 0) {
			// if no authentication steps defined - it should be the default
			// behavior.
		...
		if (authSteps != null && authSteps.length > 0) {
			...
			try {
				...
				if (ApplicationConstants.AUTH_TYPE_LOCAL
					...
					// for local authentication there can only be only one
					...
					if (authSteps.length != 1 || authSteps[0] == null
						...
						.equalsIgnoreCase(localAndOutboundAuthConfig.getAuthenticationType())) {
					// for federated authentication there can only be only one
					// authentication step
					// and only one federated authenticator - which is the
					// default authenticator of
				...
				for (AuthenticationStep authStep : authSteps) {
					...
					// an authentication step should have at least one federated
					// identity
					...
					if (federatedIdps != null && federatedIdps.length > 0) {
						...
						for (IdentityProvider federatedIdp : federatedIdps) {
							...
							// the identity provider name wso2carbon-local-idp
							// is reserved.
							if (ApplicationConstants.LOCAL_IDP.equalsIgnoreCase(idpName)) {
								throw new IdentityApplicationManagementException(
										"The federated IdP name cannot be equal to " + ApplicationConstants.LOCAL_IDP);
							}
										...
										if (authenticator != null) {
											...
											if (log.isDebugEnabled()) {
												log.debug("Updating Federated IdP of Application " + applicationId
														+ " Step Order: " + authStep.getStepOrder() + " IdP: " + idpName
	...
	public ServiceProvider getApplication(String applicationName, String tenantDomain)
		...
		try {
			...
			serviceProvider.setInboundAuthenticationConfig(
					getInboundAuthenticationConfig(applicationId, connection, tenantID));
			serviceProvider.setLocalAndOutBoundAuthenticationConfig(
					getLocalAndOutboundAuthenticationConfig(applicationId, connection, tenantID));

			serviceProvider.setInboundProvisioningConfig(
					getInboundProvisioningConfiguration(applicationId, connection, tenantID));

			serviceProvider.setOutboundProvisioningConfig(
					getOutboundProvisioningConfiguration(applicationId, connection, tenantID));
			...
			permissionAndRoleConfig.setRoleMappings(roleMappings.toArray(new RoleMapping[roleMappings.size()]));
			serviceProvider.setPermissionAndRoleConfig(permissionAndRoleConfig);

			RequestPathAuthenticatorConfig[] requestPathAuthenticators = getRequestPathAuthenticators(applicationId,
					connection, tenantID);
	...
	private ServiceProvider getBasicApplicationData(String applicationName, Connection connection, int tenantID)
		...
		try {
			...
			// ID, TENANT_ID, APP_NAME, USER_STORE, USERNAME, DESCRIPTION,
			// ROLE_CLAIM, AUTH_TYPE,
			// PROVISIONING_USERSTORE_DOMAIN, IS_LOCAL_CLAIM_DIALECT,
			...
			if (basicAppDataResultSet.next()) {
				...
				try {
					tenantDomain = ApplicationManagementServiceComponentHolder.getInstance().getRealmService()
							.getTenantManager().getDomain(basicAppDataResultSet.getInt(2));
				} catch (UserStoreException e) {
					log.error("Error while reading tenantDomain", e);
					throw new IdentityApplicationManagementException(
							"Error while reading tenant " + "domain for application " + applicationName);
				}
				...
				localAndOutboundAuthenticationConfig
						.setAlwaysSendBackAuthenticatedListOfIdPs("1".equals(basicAppDataResultSet.getString(12)));
				localAndOutboundAuthenticationConfig.setSubjectClaimUri(basicAppDataResultSet.getString(15));
				serviceProvider.setLocalAndOutBoundAuthenticationConfig(localAndOutboundAuthenticationConfig);

				serviceProvider.setSaasApp("1".equals(basicAppDataResultSet.getString(16)));

				if (log.isDebugEnabled()) {
					log.debug("ApplicationID: " + serviceProvider.getApplicationID() + " ApplicationName: "
							+ serviceProvider.getApplicationName() + " UserName: "
							+ serviceProvider.getOwner().getUserName() + " TenantDomain: "
	...
	private String getAuthenticationType(int applicationid, Connection connection) throws SQLException {
		...
		try {
			authTypeStmt = connection.prepareStatement(ApplicationMgtDBQueries.LOAD_AUTH_TYPE_BY_APP_ID);
	...
	/**
	 * This method will be heavily used by the Authentication Framework. The
	 * framework would ask for application data with the given client key and
	 * secrete
	 *
	 * @param clientId
	 * @param type
	...
	public ServiceProvider getApplicationData(String clientId, String type, String tenantDomain)
		...
		try {
			tenantID = ApplicationManagementServiceComponentHolder.getInstance().getRealmService().getTenantManager()
	...
	public String getApplicationName(int applicationID) throws IdentityApplicationManagementException {
		...
		try {
			...
			throw new IdentityApplicationManagementException("Failed loading the application with " + applicationID, e);
		} finally {
			IdentityApplicationManagementUtil.closeConnection(connection);
		}
	}
	...
	private String getApplicationName(int applicationID, Connection connection) throws SQLException {
		...
		try {
			loadBasicAppInfoStmt = connection.prepareStatement(ApplicationMgtDBQueries.LOAD_APP_NAME_BY_APP_ID);
	 ...
	 * @throws SQLException
	 */
	private InboundAuthenticationConfig getInboundAuthenticationConfig(int applicationId, Connection connection,
			int tenantID) throws SQLException {
		...
		try {
			...
			while (resultSet.next()) {
				...
				// this is done to handle empty string added to oracle database
				// as null.
				if (authKey == null) {
					authKey = "";
				}
				...
				if (log.isDebugEnabled()) {
					log.debug("Auth request key: " + inbountAuthRequest.getInboundAuthKey() + " Auth request type: "
							+ inbountAuthRequest.getInboundAuthType());
				}
			}
		...
		while (it.hasNext()) {
			...
			if (!customAuthenticatorsAlreadyIn.containsKey(entry.getKey())) {
				...
				InboundAuthenticationRequestConfig inbountAuthRequest = authRequestMap
						.get(entry.getValue().getName() + ":" + entry.getValue().getAuthKey());
		...
		inboundAuthenticationConfig.setInboundAuthenticationRequestConfigs(
				authRequestMap.values().toArray(new InboundAuthenticationRequestConfig[authRequestMap.size()]));
	 ...
	 * @throws SQLException
	 */
	private LocalAndOutboundAuthenticationConfig getLocalAndOutboundAuthenticationConfig(int applicationId,
		...
		try {
			getStepInfoPrepStmt = connection.prepareStatement(ApplicationMgtDBQueries.LOAD_STEPS_INFO_BY_APP_ID);
			...
			while (stepInfoResultSet.next()) {
				...
				if (authenticatorInfo != null && authenticatorInfo.get(ApplicationConstants.IDP_NAME) != null
						&& ApplicationConstants.LOCAL_IDP_NAME.equals(authenticatorInfo.get("idpName"))) {
					...
					localAuthenticator
							.setDisplayName(authenticatorInfo.get(ApplicationConstants.IDP_AUTHENTICATOR_DISPLAY_NAME));
					...
					List<FederatedAuthenticatorConfig> idpAuths = stepFedIdps
							.get(authenticatorInfo.get(ApplicationConstants.IDP_NAME));
			...
			try {
				...
				if (localAndOutboundConfigResultSet.next()) {
					localAndOutboundConfiguration.setUseTenantDomainInLocalSubjectIdentifier(
							"1".equals(localAndOutboundConfigResultSet.getString(1)));
					localAndOutboundConfiguration.setUseUserstoreDomainInLocalSubjectIdentifier(
							"1".equals(localAndOutboundConfigResultSet.getString(2)));
					localAndOutboundConfiguration.setAlwaysSendBackAuthenticatedListOfIdPs(
							"1".equals(localAndOutboundConfigResultSet.getString(3)));
					localAndOutboundConfiguration.setSubjectClaimUri(localAndOutboundConfigResultSet.getString(4));
				}
			} finally {
	...
	private ClaimConfig getClaimConfiguration(int applicationId, Connection connection, int tenantID)
		...
		try {
			...
			while (resultSet.next()) {
				...
				if (log.isDebugEnabled()) {
					log.debug("Local Claim: " + claimMapping.getLocalClaim().getClaimUri() + " SPClaim: "
		...
		try {
			loadClaimConfigsPrepStmt = connection.prepareStatement(ApplicationMgtDBQueries.LOAD_CLAIM_CONIFG_BY_APP_ID);
	 ...
	 * @throws IdentityApplicationManagementException
	 */
	private RequestPathAuthenticatorConfig[] getRequestPathAuthenticators(int applicationId, Connection connection,
	...
	private void deleteRequestPathAuthenticators(int applicationID, Connection connection) throws SQLException {
		...
		try {
			deleteReqAuthPrepStmt = connection.prepareStatement(ApplicationMgtDBQueries.REMOVE_REQ_PATH_AUTHENTICATOR);
	 ...
	 * @throws IdentityApplicationManagementException
	 */
	private List<RoleMapping> getRoleMappingOfApplication(int applicationId, Connection connection, int tenantID)
			throws IdentityApplicationManagementException {
		...
		ResultSet resultSet = null;
		try {
			getClientInfo = connection.prepareStatement(ApplicationMgtDBQueries.LOAD_ROLE_MAPPING_BY_APP_ID);
			// IDP_ROLE, SP_ROLE
			getClientInfo.setInt(1, applicationId);
			getClientInfo.setInt(2, tenantID);
			resultSet = getClientInfo.executeQuery();
			...
			while (resultSet.next()) {
				...
				if (log.isDebugEnabled()) {
					log.debug("Local Role: " + roleMapping.getLocalRole().getLocalRoleName() + " SPRole: "
							+ roleMapping.getRemoteRole());
				}
			}
	...
	public ApplicationBasicInfo[] getAllApplicationBasicInfo() throws IdentityApplicationManagementException {
		...
		try {
			getAppNamesStmt = connection.prepareStatement(ApplicationMgtDBQueries.LOAD_APP_NAMES_BY_TENANT);
	...
	/**
	 * Deletes the application from IDN_APPMGT_APP table. Cascade deletes with
	 * foreign key constraints should delete the corresponding entries from the
	 * tables
	 *
	 * @param appName
	 * @throws IdentityApplicationManagementException
	...
	public void deleteApplication(String appName) throws IdentityApplicationManagementException {
		...
		try {
			...
			deleteClientPrepStmt = connection.prepareStatement(ApplicationMgtDBQueries.REMOVE_APP_FROM_APPMGT_APP);
	 ...
	 * @throws SQLException
	 */
	private void deleteLocalAndOutboundAuthenticationConfiguration(int applicationId, Connection connection)
			throws SQLException {
		...
		try {
			...
			IdentityApplicationManagementUtil.closeStatement(deleteLocalAndOutboundAuthConfigPrepStmt);
		}
	}
	 ...
	 * @throws IdentityApplicationManagementException
	 */
	private void deteClaimConfiguration(int applicationID, Connection connection) throws SQLException {
	...
	public void deleteAssignedPermissions(Connection connection, String applicationName,
			ApplicationPermission[] permissions) throws IdentityApplicationManagementException, SQLException {
		...
		if (!CollectionUtils.isEmpty(removedPermissions)) {
			...
			for (ApplicationPermission applicationPermission : removedPermissions) {
				String permissionValue = ApplicationMgtUtil.PATH_CONSTANT
						+ ApplicationMgtUtil.getApplicationPermissionPath() + ApplicationMgtUtil.PATH_CONSTANT
						+ applicationName + ApplicationMgtUtil.PATH_CONSTANT + applicationPermission.getValue();
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.wso2.carbon.identity.application.mgt.dao.ApplicationDAO#
	 * getServiceProviderNameByClientId (java.lang.String, java.lang.String,
	 * java.lang.String)
	 */
	public String getServiceProviderNameByClientId(String clientId, String clientType, String tenantDomain)
			throws IdentityApplicationManagementException {
		int tenantID = -123;
	 ...
	 * @throws IdentityApplicationManagementException
	 */
	private Map<String, String> getClaimMapping(String serviceProviderName, String tenantDomain, boolean localIdpAsKey)
			throws SQLException, IdentityApplicationManagementException {
	 ...
	 * @throws SQLException
	 */
	private int getAuthentictorID(Connection conn, int tenantId, String idpName, String authenticatorName)
			throws SQLException {
	 ...
	 * @throws SQLException
	 */
	private Map<String, String> getAuthenticatorInfo(Connection conn, int tenantId, int authenticatorId)
			throws SQLException {
		PreparedStatement prepStmt = null;
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()) {
				...
				returnData.put(ApplicationConstants.IDP_AUTHENTICATOR_DISPLAY_NAME, rs.getString(3));
			}
		} finally {
			IdentityApplicationManagementUtil.closeStatement(prepStmt);
		}
	 ...
	 * @param applicationName
	 *            Application name
	 * @return Map of key value pairs. key is UM table id and value is
	 *         permission
	 * @throws SQLException
	 */
	...
	private void deleteRolePermissionMapping(int id) throws SQLException {
		...
		try {
			...
			connection = IdentityDatabaseUtil.getUserDBConnection();
			deleteRolePermissionPrepStmt = connection
					.prepareStatement(ApplicationMgtDBQueries.REMOVE_UM_ROLE_PERMISSION);

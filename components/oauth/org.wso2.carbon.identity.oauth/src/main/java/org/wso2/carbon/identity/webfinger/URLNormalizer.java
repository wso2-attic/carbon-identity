...
public class URLNormalizer {
	...
	public static WebFingerRequest normalizeResource(WebFingerRequest request) throws WebFingerEndPointException {
		...
		if (Strings.isNullOrEmpty(identifier)) {
			...
			throw new WebFingerEndPointException(WebFingerEndPointException.ERROR_CODE_INVALID_RESOURCE,"Null or empty URI.");
			...
			if (m.matches()) {
				...
				request.setFragment(m.group(15)); // we throw away the hash, but this is the group it would be if we kept it
			} else {
				// doesn't match the pattern, throw it out
				log.warn("Parser couldn't match input: " + identifier);
				throw new WebFingerEndPointException(WebFingerEndPointException.ERROR_CODE_INVALID_RESOURCE,"URI in wrong format.");
			}

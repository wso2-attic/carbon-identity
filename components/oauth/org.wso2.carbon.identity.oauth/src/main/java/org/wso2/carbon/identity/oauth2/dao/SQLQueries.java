...
public class SQLQueries {
    ...
    public static final String GET_ACCESS_TOKEN_BY_AUTHZUSER_AND_TENANTID = "SELECT DISTINCT ACCESS_TOKEN FROM " +
            "IDN_OAUTH2_ACCESS_TOKEN WHERE AUTHZ_USER=? AND TENANT_ID=?";

    public static final String GET_SESSION_IDS_FOR_CONSUMER_KEY = "SELECT ACCESS_TOKEN AS TOKEN FROM " +
            "IDN_OAUTH2_ACCESS_TOKEN WHERE TOKEN_ID IN (SELECT SESSION_ID FROM (SELECT SESSION_ID,MAX(TIME_CREATED)" +
            "FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID IN (SELECT CODE_ID FROM IDN_OAUTH2_AUTHORIZATION_CODE JOIN " +
            "IDN_OAUTH_CONSUMER_APPS ON IDN_OAUTH2_AUTHORIZATION_CODE.CONSUMER_KEY_ID =  IDN_OAUTH_CONSUMER_APPS.ID " +
            "WHERE IDN_OAUTH_CONSUMER_APPS.CONSUMER_KEY =?) OR SESSION_ID IN (SELECT TOKEN_ID FROM " +
            "IDN_OAUTH2_ACCESS_TOKEN JOIN IDN_OAUTH_CONSUMER_APPS ON IDN_OAUTH2_ACCESS_TOKEN.CONSUMER_KEY_ID =  " +
            "IDN_OAUTH_CONSUMER_APPS.ID WHERE IDN_OAUTH_CONSUMER_APPS.CONSUMER_KEY = ?)GROUP BY SESSION_ID) )UNION " +
            "ALL SELECT AUTHORIZATION_CODE AS TOKEN FROM IDN_OAUTH2_AUTHORIZATION_CODE WHERE CODE_ID IN (SELECT " +
            "SESSION_ID FROM (SELECT SESSION_ID,MAX(TIME_CREATED) FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID IN" +
            "(SELECT CODE_ID FROM IDN_OAUTH2_AUTHORIZATION_CODE JOIN IDN_OAUTH_CONSUMER_APPS ON " +
            "IDN_OAUTH2_AUTHORIZATION_CODE.CONSUMER_KEY_ID =  IDN_OAUTH_CONSUMER_APPS.ID WHERE    " +
            "IDN_OAUTH_CONSUMER_APPS.CONSUMER_KEY = ?) OR SESSION_ID IN (SELECT TOKEN_ID FROM IDN_OAUTH2_ACCESS_TOKEN" +
            " JOIN IDN_OAUTH_CONSUMER_APPS ON IDN_OAUTH2_ACCESS_TOKEN.CONSUMER_KEY_ID =  IDN_OAUTH_CONSUMER_APPS.ID " +
            "WHERE IDN_OAUTH_CONSUMER_APPS.CONSUMER_KEY = ?) GROUP BY SESSION_ID) )";

    public static final String GET_AUTHORIZATION_CODE_BY_AUTHZUSER_AND_TENANTID = "SELECT DISTINCT AUTHORIZATION_CODE" +
            " FROM IDN_OAUTH2_AUTHORIZATION_CODE WHERE AUTHZ_USER=? AND TENANT_ID=?";

 ...
 * under the License.
 */
package org.wso2.carbon.identity.oauth.endpoint.filter;
...
public class OAuthAuthenticationFilter implements RequestHandler, ResponseHandler {

    private static Log log = LogFactory.getLog(OAuthAuthenticationFilter.class);
    ...
    public Response handleRequest(Message message, ClassResourceInfo classResourceInfo) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Authenticating OAuth Authentication request..");
        }
        OAuthAuthenticatorRegistry OAuthAuthRegistry = OAuthAuthenticatorRegistry.getInstance();
        if (OAuthAuthRegistry != null) {
            OAuthAuthenticationHandler OAuthAuthHandler = OAuthAuthRegistry
                    .getAuthenticator(message, classResourceInfo);
            boolean isAuthenticated = false;
            if (OAuthAuthHandler != null) {
                isAuthenticated = OAuthAuthHandler.isAuthenticated(message, classResourceInfo);
                if (isAuthenticated) {
                    return null;
                }
            }

                ...
                if (SESSION_CONTEXT_CACHE_NAME.equalsIgnoreCase(type)) {
                    ...
                    if(entry instanceof SessionContextCacheEntry) {
                        ...
                        for (Map.Entry<String, SequenceConfig> session : sessions) {
                            String userFullName = session.getValue().getAuthenticatedUser();
                            if(userFullName.contains("/")) {
                                String[] nameParts = userFullName.split("/");
                                userName = nameParts[1];
                                userStoreDomain = nameParts[0];
                            } else {
                                userName = userFullName;
                                userStoreDomain = UserStoreConfigConstants.PRIMARY;;
                            }
                            tenantDomainName = session.getValue().getAuthenticatedUserTenantDomain();
                        }
                    }

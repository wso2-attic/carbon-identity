...
public class DefaultStepHandler implements StepHandler {
    ...
    protected void doAuthentication(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            // store authenticated user tenant domain
            // Reading user tenant domain which has been set by authenticator
            String authenticatedUserTenantDomain = (String) context.getProperty("user-tenant-domain");
            context.setProperty("user-tenant-domain", null);
            // TODO: user tenant domain has to be an attribute in the AuthenticationContext
            // String authenticatedUserTenantDomain = context.getSubjectTenantDomain();
            stepConfig.setAuthenticatedUserTenantDomain(authenticatedUserTenantDomain);
            authenticatedIdPData.setAuthenticatedUserTenantDomain(authenticatedUserTenantDomain);

            if (log.isDebugEnabled()) {
                log.debug("Setting authenticated user and tenant domain to both step config and AuthenticatedIdPData " +
                                "from idp : " + context.getExternalIdP() + ", authenticator : " + authenticator.getName());
                log.debug("Authenticated User: " + stepConfig.getAuthenticatedUser());
                log.debug("Authenticated User Tenant Domain: " + stepConfig.getAuthenticatedUserTenantDomain());
            }
    ...
    protected void populateStepConfigWithAuthenticationDetails(StepConfig stepConfig,
        ...
        stepConfig.setAuthenticatedUserTenantDomain(authenticatedIdPData.getAuthenticatedUserTenantDomain());
        ...
        if (log.isDebugEnabled()) {
            log.debug("Setting authenticated user and tenant domain to step config using previously authenticated " +
                    "idp : " + stepConfig.getAuthenticatedIdP() + ", authenticator : " + stepConfig.getAuthenticatedAutenticator());
            log.debug("Authenticated User: " + stepConfig.getAuthenticatedUser());
            log.debug("Authenticated User Tenant Domain: " + stepConfig.getAuthenticatedUserTenantDomain());
        }
    }

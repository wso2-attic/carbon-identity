                    ...
                    if (associatedID != null && associatedID.trim().length() > 0) {
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug("Setting associated authenticated user and tenant domain " +
                                    "to both step and sequence configs");
                            log.debug("Authenticated User: " + sequenceConfig.getAuthenticatedUser());
                            log.debug("Authenticated User Tenant Domain: " + sequenceConfig.getAuthenticatedUserTenantDomain());
                        }
                        ...
                        sequenceConfig.setAuthenticatedUserTenantDomain(stepConfig.getAuthenticatedUserTenantDomain());
...
//
                        if (log.isDebugEnabled()) {
                            log.debug("Setting federated authenticated user and tenant domain to sequence config");
                            log.debug("Authenticated User: " + sequenceConfig.getAuthenticatedUser());
                            log.debug("Authenticated User Tenant Domain: " + sequenceConfig.getAuthenticatedUserTenantDomain());
                        }
                    }
                ...
                if (stepConfig.isSubjectIdentifierStep()) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("Setting local authenticated user and tenant domain to sequence config");
                        log.debug("Authenticated User: " + sequenceConfig.getAuthenticatedUser());
                        log.debug("Authenticated User Tenant Domain: " + sequenceConfig.getAuthenticatedUserTenantDomain());
                    }
                }
        ...
        if (subjectClaimURI != null && !subjectClaimURI.isEmpty() && subjectValue != null) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Updating authenticated user according to subject claim uri : " + subjectClaimURI);
                log.debug("Authenticated User: " + sequenceConfig.getAuthenticatedUser());
            }

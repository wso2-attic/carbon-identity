        ...
        for (AuthenticatorConfig reqPathAuthenticator : reqPathAuthenticators) {
            ...
            if (authenticator.canHandle(request)) {
                ...
                try {
                    ...
                    // Reading user tenant domain which has been set by (request path) authenticator
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("Setting authenticated user and tenant domain to both sequence config and AuthenticatedIdPData " +
                                "from authenticator : " + authenticator.getName());
                        log.debug("Authenticated User: " + seqConfig.getAuthenticatedUser());
                        log.debug("Authenticated User Tenant Domain: " + seqConfig.getAuthenticatedUserTenantDomain());
                    }
    ...
    protected void handlePostAuthentication(HttpServletRequest request,
        ...
        if (context.getSequenceConfig().getApplicationConfig().getSubjectClaimUri() != null
            ...
            if (subjectValue != null) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Updating authenticated user according to subject claim uri : " +
                            context.getSequenceConfig().getApplicationConfig().getSubjectClaimUri().trim());

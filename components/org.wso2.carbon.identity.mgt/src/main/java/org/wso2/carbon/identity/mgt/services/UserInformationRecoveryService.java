...
public class UserInformationRecoveryService {
	...
	public VerificationBean sendRecoveryNotification(String username, String key, String notificationType)
		...
		try {
            ...
            bean = handleError(
                    VerificationBean.ERROR_CODE_INVALID_CODE + " invalid confirmation code for user : " + username, e1);
            return bean;
		}
...
//			Send email data only if not internally managed.
            ...
            bean = handleError(
                    VerificationBean.ERROR_CODE_UN_EXPECTED + " Error when sending recovery message for user: " +
                    username, e);
            return bean;
        }
		return bean;
	...
	public VerificationBean verifyConfirmationCode(String username, String code,
		...
		if (IdentityMgtConfig.getInstance().isCaptchaVerificationInternallyManaged()) {
			try {
                ...
                bean = handleError(
                        VerificationBean.ERROR_CODE_INVALID_CODE + " Error while validating captcha for user : " +
                        username, e);
                return bean;
            }
		}
		...
		try {
            ...
            bean = handleError(
                    VerificationBean.ERROR_CODE_INVALID_CODE + " Error verifying confirmation code for user : " +
                    username, e);
            return bean;
        }
	...
	public VerificationBean updatePassword(String username, String confirmationCode,
		...
		try {
            ...
            bean = handleError(
                    VerificationBean.ERROR_CODE_UN_EXPECTED + " Error while updating credential for user: " + username,
                    e);
            return bean;
		}
		return bean;
	...
	public ChallengeQuestionIdsDTO getUserChallengeQuestionIds(String username, String confirmation)
		...
		try {
            ...
            idsDTO = handleChallengeIdError("Error while getting challenge question ids for user : " + username, e);
            return idsDTO;
		}
		...
		if (bean.isVerified()) {
			try {
                ...
                idsDTO =
                        handleChallengeIdError("Error when getting user challenge questions for user : " + username, e);
                return idsDTO;
            }
		} else {
	...
	public UserIdentityClaimDTO[] getUserIdentitySupportedClaims(String dialect)
		...
		if (claims == null || claims.length == 0) {
			log.warn("Cound not find any matching claims for requested dialect : " + dialect);
...
//				Send email data only if not internally managed.
	...
	public VerificationBean registerUser(String userName, String password,
		...
		try {
            ...
            vBean = handleError(
                    VerificationBean.ERROR_CODE_UN_EXPECTED + " Error retrieving the user store manager for the tenant",
                    e);
            return vBean;
        }
		...
		try {
			...
			try {
                ...
                vBean = handleError(
                        VerificationBean.ERROR_CODE_UN_EXPECTED + " Error occurred while adding user : " + userName, e);
                return vBean;
			}
		} catch (UserStoreException e) {
            vBean = handleError(VerificationBean.ERROR_CODE_UN_EXPECTED + " Error occurred while adding user : " + userName, e);
            return vBean;
		}
	...
	public VerificationBean confirmUserSelfRegistration(String username, String code,
		...
		if (IdentityMgtConfig.getInstance().isCaptchaVerificationInternallyManaged()) {
			try {
                ...
                bean = handleError(
                        VerificationBean.ERROR_CODE_INVALID_CAPTCHA + " Error while validating captcha for user : " +
                        username, e);
                return bean;
			}
		}
		...
		try {
            ...
            bean = handleError(
                    VerificationBean.ERROR_CODE_INVALID_USER + " Error verifying user account for user : " + username,
                    e);
            return bean;
		}

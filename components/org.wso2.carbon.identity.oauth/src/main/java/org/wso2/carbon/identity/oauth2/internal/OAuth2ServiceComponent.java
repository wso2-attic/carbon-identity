...
public class OAuth2ServiceComponent {
	...
	protected void activate(ComponentContext context) {
		...
		bundleContext.registerService(OAuthServerConfiguration.class.getName(), oauthServerConfig,
		                              null);
		OAuth2TokenValidationService tokenValidationService = new OAuth2TokenValidationService();
		bundleContext.registerService(OAuth2TokenValidationService.class.getName(),
	...
	protected void setApplicationMgtService(ApplicationManagementService applicationMgtService) {
		if (log.isDebugEnabled()) {
			log.debug("ApplicationManagementService set in Identity ApplicationManagementComponent bundle");
		}
	 ...
	 *
	 * @param applicationMgtServiceImpl
	 */
	protected void unsetApplicationMgtService(ApplicationManagementService applicationMgtServiceImpl) {
		if (log.isDebugEnabled()) {
			log.debug("ApplicationManagementService unset in Identity ApplicationManagementComponent bundle");
		}

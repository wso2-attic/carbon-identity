...
public class UMAService {
    ...
    public UmaResponse getResourceSet(UmaResourceSetRegRequest umaResourceSetRegRequest) throws IdentityUMAException {
        ...
        try {
            ...
            resourceSetDO =
                    resourceSetMgtDAO.retrieveResourceSet(resourceSetId, accessTokenId, userStoreDomain);
            ...
            IdentityUMAException identityUMAException =
                    new IdentityUMAException("Unable to retrieve the token ID for access token used to create resource", e);
            identityUMAException.setErrorStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            identityUMAException.setErrorDetails("Unable to retrieve resource sets");
    ...
    public UmaResponse deleteResourceSet(UmaResourceSetRegRequest umaResourceSetRegRequest) throws IdentityUMAException {
        ...
        try {
            String accessTokenId = getAccessTokenId(accessToken, false);
            boolean isSuccessFul = resourceSetMgtDAO.removeResourceSet(resourceSetId, accessTokenId, userStoreDomain);
    ...
    public UmaResponse updateResourceSet(UmaResourceSetRegRequest umaResourceSetRegRequest) throws IdentityUMAException {
        UmaResponse.UmaResponseBuilder builder;
        ...
        ResourceSetDO newResourceSetDO = new ResourceSetDO(umaResourceSetRegRequest.getResourceSetDescriptionBean());
        newResourceSetDO.setResourceSetId(resourceSetId);
        newResourceSetDO.setTimeCreated(new Timestamp(new Date().getTime()));
        try {
            String accessTokenId = getAccessTokenId(accessToken, false);
            newResourceSetDO.setTokenId(accessTokenId);

            boolean isSuccessFul =
                    resourceSetMgtDAO.updateResourceSet(resourceSetId, newResourceSetDO, userStoreDomain);

            if (isSuccessFul) {
                builder = UmaResponse.status(HttpServletResponse.SC_OK)
                        .setParam(UMAProtectionConstants.RESOURCE_SET_ID, resourceSetId);
                if (log.isDebugEnabled()) {
                    log.debug("Resource successfully updated with ID : " + resourceSetId);
                }
            } else {
                // we could not find the resource to update, hence this error message
                builder = UmaResponse.errorResponse(HttpServletResponse.SC_NOT_FOUND)
                        .setError(UMAProtectionConstants.ERR_RESOURCE_SET_NOT_FOUND);
                log.error("Resource Set with ID: " + resourceSetId + " could not be updated");
            }

        } catch (IdentityUMAException e) {
            String errorMsg = "Error when updating resource set description with resourceSetId : " + resourceSetId;
            log.error(errorMsg, e);
            throw new IdentityUMAException(errorMsg, e);

...
public class JsonMessageModule extends AbstractNotificationSendingModule {
    ...
    public void sendMessage(PublisherEvent publisherEvent) throws NotificationManagementException {
        // Read the relevant event from configurations. (No multiple events can register)
        ...
        if (jsonSubscription != null) {
            ...
            // Send messages to each endpoint in endpoints (configured and dynamic)
            for (JsonEndpointInfo endpoint : endpoints) {
                ...
                try {
                    ...
                    entity = new StringEntity(getJSONData(jsonContent, jsonSubscription.getSubscriptionProperties(),
                            endpoint.getEndpointsParams(), publisherEvent.getEventProperties()));
                    post.setEntity(entity);

                    try {
                        response = client.execute(post);
                        log.info("Notification message has been posted to " + endpoint.getEndpoint());
                    } catch (IOException e) {
                        log.error("Error while executing POST operation", e);
                    }

                } catch (UnsupportedEncodingException e) {
                    log.error("Error while creating StringEntry from given JSON string", e);
                } finally {
                    if (log.isDebugEnabled() && response != null) {
                        log.debug("Response of HTTP post is " + response.getStatusLine());
                    }
                    post.abort();
                }
            }

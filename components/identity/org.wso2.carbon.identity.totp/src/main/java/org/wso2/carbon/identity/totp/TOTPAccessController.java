...
public class TOTPAccessController {
    ...
    public boolean isTOTPEnabledForLocalUser(String username) throws TOTPException {
        ...
        try {
            ...
            log.error("TOTPAccessController failed while trying to get the tenant ID of the use", e);
            throw new TOTPException(e.getMessage(),e);
        } catch (UserStoreException e) {
            log.error("TOTPAccessController failed while trying to access userRealm", e);
            throw new TOTPException(e.getMessage(),e);
        } catch (IdentityApplicationManagementException e) {
            log.error("TOTPAccessController failed while trying to access encoding method", e);
            throw new TOTPException(e.getMessage(),e);
        }
    }
}

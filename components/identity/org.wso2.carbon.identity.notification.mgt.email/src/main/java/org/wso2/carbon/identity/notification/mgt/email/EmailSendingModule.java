...
/**
 * This class will be the registering class as a service for email sending module on message
 * sending component. This is responsible for sending email messages on published events taking
 * care of configured information and dynamic information which are sent by the publisher
 */
...
public class EmailSendingModule extends AbstractNotificationSendingModule {
    ...
    public void sendMessage(PublisherEvent publisherEvent) throws NotificationManagementException {
        ...
        // Message sending will only be done if there is a registered event on this module
        if (subscription != null) {
            List<EmailEndpointInfo> endpointInfoList = new ArrayList<EmailEndpointInfo>(subscription.
                    getEmailEndpointInfoList());
            ...
            try {
                ...
                for (EmailEndpointInfo endpointInfo : endpointInfoList) {
                    ...
                    // particular endpoint.
                    if (template == null) {
                        log.error("No template found to send email to " + endpointInfo.getEmailAddress() + ", " +
                                "sending aborted");
                        continue;
                    }
                    // Get the message which the place holders are replaced by configurations and
                    // subscription properties.
                    String message = getMessage(template, subscription.getSubscriptionProperties(),
                            endpointInfo.getProperties(), publisherEvent.getEventProperties());
                    ...
                    options.setTo(new EndpointReference(EmailModuleConstants.MAILTO_LABEL + endpointInfo.
                            getEmailAddress()));
                ...
                log.error("Error while sending email", axisFault);
            } finally {

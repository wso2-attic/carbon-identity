...
/**
 * @scr.component name="carbon.message.email" immediate="true"
 */
    ...
    protected void activate(ComponentContext ctxt) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Email sending module is activated");
            }
            // Catching throwable, since there may be throwable throwing while initiating
        } catch (Throwable t) {
            log.error("Error while registering email sending module", t);
        }
    }

    protected void deactivate(ComponentContext ctxt) {
        if (log.isDebugEnabled()) {
            log.debug("Email sending module is deactivated");
        }
    }
}
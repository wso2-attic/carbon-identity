...
<div id="middle">
    ...
    <div id="workArea">
        ...
        <div style="padding-bottom: 10px;">
            ...
            <div
                    style="padding-left: 10px; color: <%=item.getType().equals("SUCCESS")
                     ? "#029219" : "#E01212"%>">
                <%=!(item.getType().equals("SUCCESS")) ? "Line Number: " + item.getLineNumber() + "-->" : ""%>
                <%=item.getMessage()%>
            </div>
        </div>
        <%
    ...
    <table class="styledLeft noBorders" style="width: 100%">
        <thead>
        <tr>
            <th colspan="2">Import Entitlement Policy</th>
        </tr>
        </thead>
        <tbody>
        <tr>
            <td width="30%">
                Import Entitlement Policy From
                <select onchange="selectPolicyImportMethod();" id="importingMethod"
                        name="importingMethod">
                    <%
                        for (String importingMethod : importingMethods) {
                            if (importFrom.equals(importingMethod)) {
                    %>
                    <option value="<%=importingMethod%>" selected="selected"><%=importingMethod%>
                    </option>
                    <%
                    } else {
                    %>
                    <option value="<%=importingMethod%>"><%=importingMethod%>
                    </option>
                    <%
                            }
                        }
        ...
        <tr>
            <td>
                <form name="importPolicy" id="importPolicy" target="_self"
                      action="../../fileupload/service"
                      method="POST"
                        <%
                    ...
                    %>
                    <tr>
                        <td><input type="file" id="policyFromFileSystem" name="policyFromFileSystem"
                                   size="50"/>
                        </td>
                    </tr>
                    <%
                    ...
                    <tr>
                        <td>
                            <table>
                                ...
                                <tr>
                                    <td width="5%">
                                        <input type="text" name="policyFromRegistry"
                                               id="policyFromRegistry" readonly="readonly"/>
                                    </td>
                                    <td>

...
public class SQLConstants {
    ...
    public static final String UPDATE_WORKFLOW_QUERY =
            "UPDATE WF_WORKFLOW SET WF_NAME=?, DESCRIPTION=?, TEMPLATE_ID=?, " +
            "IMPL_ID=?  WHERE ID=? ";
    ...
    public static final String UPDATE_WORKFLOW_PARAMS_QUERY = "UPDATE WF_WORKFLOW_CONFIG_PARAM SET " +
                                                              ...
                                                              "PARAM_HOLDER=? WHERE WORKFLOW_ID=?";

    public static final String DELETE_WORKFLOW_PARAMS_QUERY =
    ...
    public static final String GET_ASSOCIATIONS_FOR_EVENT_QUERY = "SELECT WF_WORKFLOW_ASSOCIATION.WORKFLOW_ID, " +
                                                                  ...
                                                                  "WF_WORKFLOW_ASSOCIATION.EVENT_ID = ? AND " +
                                                                  "WF_WORKFLOW_ASSOCIATION" +
                                                                  ".WORKFLOW_ID = WF_WORKFLOW.ID AND WF_WORKFLOW" +
                                                                  ".TENANT_ID = ? AND WF_WORKFLOW_ASSOCIATION" +
                                                                  ".IS_ENABLED = '1'";
    ...
    public static final String GET_WORKFLOW_PARAMS =
            "SELECT WORKFLOW_ID, PARAM_NAME, PARAM_VALUE, PARAM_QNAME, PARAM_HOLDER FROM WF_WORKFLOW_CONFIG_PARAM " +
            "WHERE WORKFLOW_ID = ?";
    ...
    public static final String ADD_BPS_PROFILE_QUERY =
            "INSERT INTO WF_BPS_PROFILE(PROFILE_NAME, HOST_URL_MANAGER, HOST_URL_WORKER, USERNAME, PASSWORD, " +
            "CALLBACK_USERNAME," +
            " CALLBACK_PASSWORD, TENANT_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    public static final String UPDATE_BPS_PROFILE_QUERY =
            "UPDATE WF_BPS_PROFILE SET HOST_URL_MANAGER=?, HOST_URL_WORKER=?, USERNAME=?, PASSWORD=?, " +
            "CALLBACK_USERNAME=?, " +
    ...
    public static final String GET_BPS_PROFILE_FOR_TENANT_QUERY =
            "SELECT HOST_URL_MANAGER, HOST_URL_WORKER, USERNAME,PASSWORD,  " +
    ...
    public static final String LIST_BPS_PROFILES_QUERY =
            "SELECT PROFILE_NAME, HOST_URL_MANAGER, HOST_URL_WORKER, USERNAME, PASSWORD, CALLBACK_USERNAME, " +
            "CALLBACK_PASSWORD FROM " +
    ...
    public static final String GET_PENDING_RELATIONSHIPS_OF_ENTITY = "SELECT WF_REQUEST_ENTITY_RELATIONSHIP" +
                                                                     ".ENTITY_NAME,WF_REQUEST_ENTITY_RELATIONSHIP" +
                                                                     ".REQUEST_ID  FROM WF_REQUEST," +
                                                                     "WF_REQUEST_ENTITY_RELATIONSHIP " +
                                                                     "WHERE WF_REQUEST.UUID = " +
                                                                     "WF_REQUEST_ENTITY_RELATIONSHIP.REQUEST_ID  AND " +
                                                                     "WF_REQUEST_ENTITY_RELATIONSHIP" +
                                                                     ".ENTITY_TYPE = ? AND " +
                                                                     "WF_REQUEST_ENTITY_RELATIONSHIP.ENTITY_NAME = ? " +
                                                                     "AND WF_REQUEST.STATUS = ? AND " +
    ...
    public static final String GET_PENDING_RELATIONSHIPS_OF_GIVEN_TYPE_FOR_ENTITY = "SELECT " +
                                                                                    ...
                                                                                    "WF_REQUEST_ENTITY_RELATIONSHIP" +
                                                                                    ".ENTITY_NAME = ? AND " +
                                                                                    "WF_REQUEST.STATUS = ? AND " +
                                                                                    "WF_REQUEST.OPERATION_TYPE = ? " +
                                                                                    "AND " +
                                                                                    "WF_REQUEST_ENTITY_RELATIONSHIP" +
                                                                                    ".TENANT_ID = ?";

    public static final String GET_REQUESTS_OF_TWO_ENTITIES = "SELECT TABLE1.REQUEST_ID from " +
                                                              "WF_REQUEST_ENTITY_RELATIONSHIP AS TABLE1, " +
                                                              "WF_REQUEST_ENTITY_RELATIONSHIP AS TABLE2 WHERE TABLE1" +
                                                              ".REQUEST_ID = TABLE2.REQUEST_ID AND TABLE1.ENTITY_NAME" +
                                                              " = ? AND TABLE1.ENTITY_TYPE= ? AND TABLE2" +
    ...
    public static final String GET_REQUESTS_OF_USER_FILTER_FROM_CREATED_TIME = "SELECT UUID, OPERATION_TYPE, " +
                                                                               ...
                                                                               "REQUEST, CREATED_BY FROM WF_REQUEST " +
                                                                               "WHERE CREATED_BY = ? AND CREATED_AT >" +
                                                                               " ? AND " +
                                                                               "CREATED_AT < ? AND TENANT_ID = ? " +
                                                                               "LIMIT ?";

    public static final String GET_REQUESTS_OF_USER_FILTER_FROM_CREATED_TIME_AND_STATUS = "SELECT UUID, " +
                                                                                          "OPERATION_TYPE, " +
                                                                                          "CREATED_AT, UPDATED_AT, " +
                                                                                          "STATUS, REQUEST, " +
                                                                                          "CREATED_BY FROM WF_REQUEST" +
    ...
    public static final String GET_REQUESTS_OF_USER_FILTER_FROM_UPDATED_TIME = "SELECT UUID, OPERATION_TYPE, " +
                                                                               ...
                                                                               " ? AND " +
                                                                               "UPDATED_AT < ? AND TENANT_ID = ? " +
                                                                               "LIMIT ?";

    public static final String GET_REQUESTS_OF_USER_FILTER_FROM_UPDATED_TIME_AND_STATUS = "SELECT UUID, " +
                                                                                          "OPERATION_TYPE, " +
                                                                                          "CREATED_AT, UPDATED_AT, " +
                                                                                          "STATUS, REQUEST, " +
                                                                                          "CREATED_BY FROM WF_REQUEST" +
    ...
    public static final String GET_REQUESTS_FILTER_FROM_CREATED_TIME = "SELECT UUID, OPERATION_TYPE, " +
                                                                       "CREATED_AT, UPDATED_AT, STATUS, REQUEST, " +
                                                                       "CREATED_BY FROM WF_REQUEST WHERE CREATED_AT >" +
                                                                       " ? AND CREATED_AT < ? AND " +
                                                                       "TENANT_ID = ? LIMIT ?";

    public static final String GET_REQUESTS_FILTER_FROM_CREATED_TIME_AND_STATUS = "SELECT UUID, OPERATION_TYPE, " +
                                                                                  "CREATED_AT, UPDATED_AT, STATUS, " +
                                                                                  "REQUEST, CREATED_BY FROM " +
                                                                                  "WF_REQUEST WHERE CREATED_AT > ? " +
                                                                                  "AND CREATED_AT < ? AND " +
                                                                                  "TENANT_ID = ? AND STATUS = ? LIMIT" +
                                                                                  " ?";

    public static final String GET_REQUESTS_FILTER_FROM_UPDATED_TIME = "SELECT UUID, OPERATION_TYPE, " +
                                                                       "CREATED_AT, UPDATED_AT, STATUS, REQUEST, " +
                                                                       "CREATED_BY FROM WF_REQUEST WHERE UPDATED_AT >" +
                                                                       " ? AND UPDATED_AT < ? AND " +
                                                                       "TENANT_ID = ? LIMIT ?";

    public static final String GET_REQUESTS_FILTER_FROM_UPDATED_TIME_AND_STATUS = "SELECT UUID, OPERATION_TYPE, " +
                                                                                  "CREATED_AT, UPDATED_AT, STATUS, " +
                                                                                  "REQUEST, CREATED_BY FROM " +
                                                                                  "WF_REQUEST WHERE UPDATED_AT > ? " +
                                                                                  "AND UPDATED_AT < ? AND " +
                                                                                  "TENANT_ID = ? AND STATUS = ? LIMIT" +
                                                                                  " ?";
    ...
    public static final String GET_WORKFLOWS_OF_REQUEST = "SELECT WF_WORKFLOW.ID, WF_WORKFLOW.WF_NAME , " +
                                                          "WF_WORKFLOW_REQUEST_RELATION.STATUS, " +
                                                          "WF_WORKFLOW_REQUEST_RELATION.UPDATED_AT FROM WF_WORKFLOW, " +
                                                          "WF_WORKFLOW_REQUEST_RELATION WHERE WF_WORKFLOW.ID = " +
                                                          "WF_WORKFLOW_REQUEST_RELATION.WORKFLOW_ID AND" +
